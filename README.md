...
Descompone de forma muy rudimentaria un guión SQL en sentencias individuales. Debería funcionar si se tiene la precaución de escribir los ";" que separan sentencias a final de línea; si no se usan las palabras reservadas `BEGIN` y `END` en comentarios, cadenas, etc; y, si los bloques se construyen con el estándar `CASE` y no con `IF`.
...
Comprueba si una base de datos no tiene definido ningún esquema.
