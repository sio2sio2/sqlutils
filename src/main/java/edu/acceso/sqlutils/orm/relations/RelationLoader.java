...
public abstract class RelationLoader<E extends Entity> {
    ...
    /**
     * Datos que caracterizan al {@link DaoFactory} a partir de cual se cró el DAO que creó este cargador de relaciones.
     */
     ...
     * @param key Clave del DAO a partir del cual se crea el cargador de relaciones
     ...
     */
    public RelationLoader(String key, Class<E> entityClass, RelationLoader<? extends Entity> previous) throws DataAccessException {
        data = DaoFactory.get(key).getDaoData();
        this.entityClass = entityClass;
        this.previous = previous;
        dao = DaoFactory.get(key).getDao(this);
        logger.debug("Creado cargador de relaciones para una entidad '{}'", entityClass.getSimpleName());
    }

    /**
     * Clave del DAO a partir del cual se creó este cargador de relaciones.
